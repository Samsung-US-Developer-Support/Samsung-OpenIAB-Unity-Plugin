...
public class OpenIABEventManager : MonoBehaviour
{
     ...
     * TODO
     */ 
    public static event Action<List<SkuDetails>> querySkuListSucceededEvent;

    /**
     * TODO
     */
    public static event Action<string> querySkuListFailedEvent;
	
    /**
     * TODO
     */
    public static event Action<string> mapSkuFailedEvent;	
...
#pragma warning restore 0067
    ...
    {
		if(mapSkuFailedEvent != null){
				mapSkuFailedEvent(exception);
		}
    }
    ...
    {
        if (querySkuListFailedEvent != null)
            querySkuListFailedEvent(error);
    }
	
	private void OnQuerySkuListSucceeded(string json)
    {
        if (querySkuListSucceededEvent != null)
        {
			List<SkuDetails> skuList = new List<SkuDetails>();
			var j = new JSON(json);
			foreach (var entry in (List<object>) j.fields["skuList"])
            {
                List<object> skuItem = (List<object>) entry;
                SkuDetails value = new SkuDetails(skuItem[0].ToString());
				skuList.Add(value);
			}
            querySkuListSucceededEvent(skuList);
        }
    }
