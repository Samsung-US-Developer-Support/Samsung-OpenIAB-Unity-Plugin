...
#endif
    {
        ...
        public void queryInventory(params object[] extraParams)
        {
			...
			if(null!=extraParams && extraParams.Length>0){
				string[] extra = new string[extraParams.Length];	
				for(int i =0; i<extraParams.Length ;i++){
					try{
						extra[i] = (string) extraParams[i];
					}catch(Exception e){
						Debug.Log (e.Message);
					}
				}
				args[0].l = ConvertToStringJNIArray(extra);
			}
            methodId = AndroidJNI.GetMethodID(_plugin.GetRawClass(), "queryInventory", "([Ljava/lang/Object;)V");
		 ...
		 * 	@param extraParams this allows additional parameters if needed. Function will still work with legacy code.
		 **/
        public void queryInventory(string[] skus, params object[] extraParams)
        {
            queryInventory(skus, skus, extraParams);
        }

        private void queryInventory(string[] inAppSkus, string[] subsSkus, params object[] extraParams)
        {
            if (!IsDevice())
            {
                return;
			...
			if(null!=extraParams && extraParams.Length>0){
				...
				args[2].l = ConvertToStringJNIArray(extra);
			}
            IntPtr methodId = AndroidJNI.GetMethodID(_plugin.GetRawClass(), "queryInventory", "([Ljava/lang/String;[Ljava/lang/String;[Ljava/lang/Object;)V");
            AndroidJNI.CallVoidMethod(_plugin.GetRawObject(), methodId, args);
        }
        ...
        public void querySkuList(string skuType)
        {
            if (!IsDevice())
            {
                return;
        ...
        {
            _plugin.Call("samsungIapMode", mode);
        }
		//TODO
		public void samsungForcedMode(bool enabled){
			_plugin.Call("samsungForcedMode", enabled);
		}
